...
| **B-45** | **Ein Chef konnte ein Konto in einen fremden Betrieb verschieben.** Die Regel für `users` prüfte beim Chef nur die Firma VOR der Änderung (`gleicheFirma()`); `update({firma: B})` am eigenen Mitarbeiter — oder an einem eigenen Zweitkonto — ging durch, und das Konto las danach alles in Betrieb B. Beim Planen von Runde 117 aufgefallen, im Emulator bestätigt, bevor die Regel geändert wurde. Jetzt darf sich `firma` nur noch zurück auf `'_ohne'` bewegen (Ablehnen) | 25.9.2026, `tests/rules/konto-ohne-firma.test.js` |
